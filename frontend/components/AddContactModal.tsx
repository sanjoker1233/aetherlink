...
export function AddContactModal({ open, onClose }: Props) {
  ...
  const handleManual = async () => {
    setError('')
    if (!fp.trim() || !/^[0-9A-Fa-f]{8}$/.test(fp.trim())) {
      setError('Invalid fingerprint (8 hex characters)'); return
    }
          ...
          >
            <GlassCard hover={false} className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold neon-text">Add a contact</h2>
              ...
              {mode === 'manual' && (
                <div className="space-y-3">
                  ...
                  />
                  <GlassInput
                    placeholder="Fingerprint (8 hex characters)"
                    value={fp}
